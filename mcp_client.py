...
async def create_mcp_client(mcp_config: MCPServerConfig, exit_stack: AsyncExitStack):
    try:
        sessions = await _create_mcp_server_session(mcp_config, exit_stack)
        print(sessions)
...
async def _create_mcp_server_session(
    mcp_server_configs: MCPServerConfig, exit_stack: AsyncExitStack
):
    ...
    discover_and_create_session = [
        _connect_and_discover(discovered_servers[server_name], exit_stack)
...
async def _connect_and_discover(
    connection: Connection, exit_stack: AsyncExitStack
) -> ClientSession:
    ...
    match transport_type:
        case "stdio":
            stdio_transport = await exit_stack.enter_async_context(
                stdio_client(StdioServerParameters(**connection))
            )
    ...
    if session is None:
        raise UnboundLocalError(
            f"ClientSession not defined to this transport type {transport_type}"
        )

    await session.initialize()
    response = await session.list_tools()
    logger.info(
        f"\nConnected to server with tools: {', '.join([tool.name for tool in response.tools])}"
    )
    return session
